...
import { changeSection, resetFilter, setParams } from '../../store/reducers/filterSlice';
...
export const Catalog = () => {
	...
	const brandParam: Brand | null = urlParams.brand
		? section === Section.Tires
			...
			: section === Section.Disks
				...
				: null
		: null;
	const title = `${ urlParams.sezon ?
		t(SeasonTransform(urlParams.sezon)?.name || '', true) : '' }
		${ t(section, !(urlParams.sezon && urlParams.brand)) } ${ brandParam?.label ?? '' }
		${ urlParams.width ?? '' }${ urlParams.width && urlParams.height ? '/' : '' }${ urlParams.height ?? '' }
		${ urlParams.radius ? 'R' + urlParams.radius : '' }`;
	...
	const titleDefault = `${ title } - ${ lang === Language.UA ?
		'Купити гуму за найкращою ціною в Україні' :
		'Купить резину по лучшей цене в Украине' } |
		${ settings.ua.config_name }`;
	...
	return <LayoutWrapper>
		...
		<div className='md:pt-2 pb-5 lg:flex'>
