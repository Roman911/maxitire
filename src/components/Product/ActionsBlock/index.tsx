...
export const ActionsBlockComponent: FC<ActionsBlockProps> = ({ className, isBookmarks, handleClickBookmarks, openModal }) => {
	const url = window.location.href;

	return <div className={ classNames('gap-1.5 xl:gap-2.5 h-full', className) }>
		<button onClick={ () => handleClickBookmarks() } className={ twMerge('hover:text-blue-500', isBookmarks && 'text-blue-500') }>
			{ isBookmarks ? <i className='icon iconfont-heart'></i> : <i className='icon iconfont-heart-o'></i> }
		</button>
		<div className='group cursor-pointer relative'>
			<i className='icon iconfont-share'></i>
		...
		<button onClick={ () => openModal('Callback') } className='text-gray-700 hover:text-blue-500'>
			<i className='icon iconfont-phone'></i>
		</button>
		<button onClick={ () => openModal('AddAsk') } className='hover:text-blue-500'>
